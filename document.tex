...
smartphones has intensifed every year.In 2012, the number of smartphones in the world has reached one billion,
according to Bloomberg\cite{bloomberg} \newline
...
`Real Time Strategy`(RTS)\cite{rts}\cite{rts2}\cite{rts3}\cite{rts4} games. We
will do that with a subgenre of RTS, called Real Time
Tactics(RTT)\cite{rtt}\cite{rttvsrts}.\newline
...
and \textbf{battle tactics}.\cite{rts}\cite{rts2}\cite{rts3}\cite{rts4}\newline
...
Turn Based Strategy games\cite{tbs}\cite{rtsvstbs}(such as chess and board
...
for mobile devices. Some notable implementations are the ones of Scotland Yard,
Catan and Monopoly.\newline
...
Location-based games\cite{lbg} take advantage of the mobile devices' built-in
receivers for global positioning. They provide the user's location with an accuracy
...
ARIS\cite{aris}, Tourality\cite{tourality}, Wherigo\cite{wherigo},
conTAGion\cite{contagion}, Shadow Cities\cite{shadowcities},
SCVNGR\cite{scvngr}, Please Stay Calm\cite{pleasestaycalm}, Parallel Mafia
\cite{parallelmafia}, Parallel Kingdom\cite{parallelkingdom},
Tripventure\cite{tripventure}, Warfinger\cite{warfinger}, Totem\cite{totem},
Portal Hunt\cite{portalhunt}, aMazing\cite{amazing}, Ingress\cite{ingress},
MobileWar\cite{mobilewar}, Mister X Mobile\cite{misterxmobile}, Mobilis
XHunt\cite{mobilisxhunt}, Own This World\cite{ownthisworld},
MapAttack\cite{mapattack}.\newline
...
\begin{enumerate}
  ...
  also implemented\cite{adv} (eg. the ARIS-based game 'Mentira').
  ...
  based on multiplayer.\cite{mmo} Subgenres include MMO Role Playing Games and
  MMO Shooters.
  ...
  gameplay and requiring no specialized skills.\cite{cas} 
  ...
  a number of checkpoints.\cite{rac}
...
A special category is represented by Mister X Mobile\cite{misterxmobile} and Mobilis
XHunt\cite{mobilisxhunt}, which both bring a board game (Scotland Yard) to the mobile
environment. While the former adapts the board game to real-time
gameplay(placing it closer to the 'Multiplayer Racing Games', with elements from
'Real Time Strategy Games'), the latter falls in the definition of 'Turn Based
Strategy' games.
...
performance\cite{nativevscrossplatform}\cite{nativevscrossplatform2}. In this
case, this approach is disadvantageous. Therefore, the choice of native app
...
The server will be written using NodeJS\cite{nodejs}\cite{nodejs2}. NodeJS is a
javascript framework based on the Google V8 Engine that is designed for web
...
teamwork, without requiring specialized equipment(Airsoft\cite{airsoft}) or
highly developed skills(computer Real Time Strategy games).
