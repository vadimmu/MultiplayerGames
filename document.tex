...
following attributes: range, cooldown, duration, damage. \newline

By default weapons have instant effect(therefore no duration) and powerups have
no damage(but they have a duration) - the only exception is the 'Heal' ability.
Both weapons and cooldowns have a range - an area of effect for their use. If a
target falls within that area, the weapon or powerup of choice can be used.
After the activation of a weapon or powerup, it will be availavbe for use after
a time given by its 'cooldown' attribute. In the case of powerups('Invisibility'
and 'Shield'), their time span during which they are in effect after activation
will be given by the 'duration' attribute. \newline
...
Being something new, this game will require a few usage guidelines : \newline
\begin{enumerate}
...
The controls are separated in six areas :\newline


1. The bottom area : the Weapons
: For each weapon there is a separate button. Each button serves three purposes
: If you click it, it shoots. If you click it for a longer time, it draws the
range of the weapon on the map. Once you have used a weapon, the button is
temporarily disabled and shows the weapon cooldown countdown. \newline

2. The
down-right corner : The target selection buttons : You can select your targets
(both friends and enemies) by clicking on their markers on the map. As an
alternative, three buttons are there to help you : the Friends/Enemies toggle
button, with which you choose whether to select between the next/previous
friends or enemies. Above and below the toggle button are the 'Next Target' and
'Prev Target'(Previous Target) buttons. By pressing the 'Next' button, you
select the next closest player (If there is one selected, the next closest one
will be chosen. If nobody is selected, the closest one will be chosen.). The
'Prev Target' button gets you the opposite : the previous farthest friend or foe
is selected, according to the same logic as with 'Next Target'. Above the three
buttons, you can find a text view which shows the distance to the selected
target. !!! If you want to select yourself (for applying powerups), then you
must have no friend or foe selected. For this, you have to tap on a random,
unoccupied area on the map !!! 

3. The top-left corner : The health and messages
buttons : Your health is shown in large font. Below it, you can find the message
selection dropdown menu. This has been arranged so that you don't waste a lot of
time typing, but instead send critical preset messages to your team.

4. The top-right corner : The current position button : If you click it, the map
moves to have your position in the center.

5. The top-center area : The powerup duration views : If you enable a powerup or
somebody uses a powerup on you (for now, this applies only to 'Shield'), you
will see it's remaining duration of the effect as a countdown on the top of the
screen.

6. The area above the weapon buttons : The information area : Whenever you
shoot, are shot, send or receive a message a 'Toast' will appear with info.
